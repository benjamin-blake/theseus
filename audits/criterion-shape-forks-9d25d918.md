...
### Addendum 2026-09-22 (in-session, not by the audited model): CSF-08

A live consumer of declared plan-to-criterion links was found after the audit closed. `compute_followon_state` (`scripts/platform_roadmap_state.py:85-135`, called from `scripts/session/preflight.py:350`) globs every `docs/plans/PLAN-*.yaml` (454 at `5ce6e4b5`, 152 parsed after the `closes_criteria` substring skip) on every `compute_state_dict` call to decide whether an open criterion on an `in_progress` item already has a merged plan in flight. Measured: 4.42s of a 4.36s call; roadmap-side gate evaluation ~0s; YAML load 0.55s. It needs declared links from plans that have merged but not yet implemented, which is exactly the window Decision 197 clause 5 excludes from edge durability. `ops_execution_plans` today carries `plan_text` and `steps`, not a `closes_criteria` column, and its merge key is `rec_id` alone.

The `merge_authored_edges_lossy` assessment above (near_triggered false) stands only under the assumption that T4.5's git-to-warehouse ETL projects `closes_criteria` as plan-row content -- `PlanDocument.closes_criteria` is a first-class field, so that is the natural projection, and it satisfies this consumer without edges. If the ETL carried execution metadata alone the trigger would bite. T4.5 is deferred_post_mvp behind T4.2, and Decision 87's 2026-08-19 amendment keeps human-plan residency timed to T4.6, so the control is planned, not live. An earlier in-session conclusion that "the migration does not touch this hotspot" was verified against T4.23 (roadmap criteria) only and does not hold for T4.5 (plans).

Interim remedies are filed as recommendations and do not wait on the migration: rec-4006 (session-scoped cache at the `compute_state_dict` output boundary; never an index over `docs/plans/`) and rec-4007 (early-return guard when no item is `in_progress`, dependency edge on rec-4006). Findings now number eight; CSF-08 is medium, planned-insufficient against T4.5/T4.6/Decision 197/Decision 87, HYPOTHESIS confidence.
