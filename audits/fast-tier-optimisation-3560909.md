...
| Candidate 1 checkpoint - primary-session heavy-dependency classification | Class 2 - identical work, lower cost | The real primary pytest session suppresses only positively identified excluded-and-absent dependency collection reports, records deferrals through xdist worker output, and rolls back coverage produced while importing a deferred module. The former collect-only implementation remains only as a frozen compatibility surface. | Unknown pending a clean corrected-ref A/B; no timing claim | **Pending - no elimination claim.** The permanent-ref differ from `9cecaab2` to `7f1fa195` completed all 16 cases and compared 27,878 union-node observations. It found 72 node-verdict changes across eight cases, so that checkpoint is not accepted. All changes were fixture-compatibility failures: fast-tier subprocess doubles omitted the new plugin completion side effect, and a synthetic verifier repo omitted the two new `_pytest_diff` dependency modules. Corrections are pending a new committed-ref differ. | **Pending corrected-ref proof.** The completed checkpoint differ found zero gate-verdict changes and identical deferral maps in all 16 cases, but its 72 node-verdict changes prevent an equivalence claim. | 102 original focused tests passed. The correction passes all 48 affected orchestrator tests and all four affected verifier-differential tests; a 368-test focused run passed 367 tests, with its one sandbox-denied Git-worktree test passing on an authorised rerun. Ruff and format checks pass. Ten named baseline/candidate mutations are defined; the original eight passed against the checkpoint refs, while the two correction probes await a committed candidate ref. | High until the corrected committed-ref differ reports zero node/gate differences and matching deferral maps. Candidate 2 has not started. |
